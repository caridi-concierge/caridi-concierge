import type { CoverageTier } from "@/content/locations/types";
...
export default function LocationCoverage({ coverage }: Props) {
        ...
        <div className="grid gap-10 lg:gap-20 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.6fr)] items-start">
          <div>
            ...
            <p className="mt-5 font-body text-[14px] leading-[1.6] text-ink/55 max-w-[360px]">
              Manhattan and Brooklyn are routine. Outside the map, ask.
            </p>
          </div>
          <dl className="m-0">
            {coverage.map((c, i) => (
              <div
                key={c.tier}
                className={`grid gap-3 sm:grid-cols-[140px_1fr] sm:gap-8 py-5 ${
